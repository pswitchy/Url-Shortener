...
const HomePage = () => {
  ...
  useEffect(() => {
    // On mount, check local storage for dark mode preference
  ...
  useEffect(() => {
    // Update local storage and html class when darkMode state changes
  ...
  const shortenUrl = async () => {
    ...
    try {
      const data = await postData(API_BASE_URL, { url: longUrl });
    ...
    if (result.type === 'shorten') {
      content = (
        <div className="p-4 bg-white dark:bg-dark-bg rounded-md shadow-md transition-opacity duration-300" style={{ opacity: result ? 1 : 0 }}>
          ...
          <p className="mb-1 text-gray-700 dark:text-dark-text"><span className="font-medium">Short Code:</span> <code className="bg-gray-100 dark:bg-dark-secondary dark:text-dark-text px-2 py-1 rounded">{result.data.shortCode}</code></p>
          <p className="mb-2 text-gray-700 dark:text-dark-text"><span className="font-medium">Short URL:</span> <a href={`/r/${result.data.shortCode}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline dark:text-blue-400">{`${window.location.origin}/r/${result.data.shortCode}`}</a></p>
          <p className="mb-1 text-gray-700 dark:text-dark-text"><span className="font-medium">Original URL:</span> {result.data.url}</p>
        </div>
      );
    } else if (['retrieve', 'update', 'stats'].includes(result.type)) {
      content = (
        <div className="p-4 bg-white dark:bg-dark-bg rounded-md shadow-md transition-opacity duration-300" style={{ opacity: result ? 1 : 0 }}>
  ...
  return (
    <div className="min-h-screen py-10 flex flex-col items-center justify-start bg-gradient-to-br from-neutral-light to-white dark:from-dark-bg dark:to-gray-900 transition-colors duration-300">
      ...
      <footer className="mt-12 text-center text-gray-500 dark:text-dark-secondary">
        <p>© {new Date().getFullYear()} Vibrant URL Shortener. Made with Next.js & Tailwind CSS.</p>
      </footer>
    </div>
  );
};
